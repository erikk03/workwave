...
// Integrated matrixFactorization.js
export class MatrixFactorization {
    constructor(numUsers, numListings, numFactors, learningRate = 0.01, lambda = 0.02, iterations = 1000) {
        this.numUsers = numUsers;
        this.numListings = numListings;
        ...
        // Rating matrix
        this.R = Array(numUsers).fill().map(() => Array(numListings).fill(0)); 

        // Content-based feature matrix (additional factors for each item)
        this.contentFactors = Array(numListings).fill().map(() => Array(numFactors).fill(0));
    }

    // Initialize ratings with interaction data
    initializeRatings(interactions, userIdToIndex, listingIdToIndex) {
        interactions.forEach(({ userId, listingId, interaction }) => {
            const userIndex = userIdToIndex[userId.toString()];
            const listingIndex = listingIdToIndex[listingId.toString()];
    ...
    // Initialize content-based features (extract features from listings)
    initializeContentFeatures(listings) {
        listings.forEach(({ listingId, skillsRequired, location, employmentType }, index) => {
            const encodedFeatures = this.encodeFeatures(skillsRequired, location, employmentType);
            this.contentFactors[index] = encodedFeatures;
        });
    }

    // Encode listing features into a numerical vector
    encodeFeatures(skills, location, employmentType) {
        let features = new Array(this.numFactors).fill(0);

        skills.forEach(skill => {
            features[skill.length % this.numFactors] += 1; // Simple hash based on string length
        });

        features[location.length % this.numFactors] += 1;
        features[employmentType.length % this.numFactors] += 1;
    ...
    // Training method with combined collaborative and content-based filtering
    train() {
        let previousRMSE = Number.MAX_VALUE;
        let stableCount = 0;
                ...
                for (let j = 0; j < this.numListings; j++) {
                    if (this.R[i][j] > 0) {
                        ...
                        for (let k = 0; k < this.numFactors; k++) {
                            let userFactor = this.userFactors[i][k];
                            let itemFactor = this.itemFactors[j][k];
                            let contentFactor = this.contentFactors[j][k];

                            // Update user and item factors based on collaborative filtering error
                            this.userFactors[i][k] += this.learningRate * (2 * error * itemFactor - this.lambda * userFactor);
                            this.itemFactors[j][k] += this.learningRate * (2 * error * userFactor - this.lambda * itemFactor);

                            // Update item factors based on content-based features
                            this.itemFactors[j][k] += this.learningRate * (contentFactor - this.lambda * itemFactor);
                        }
                    }
                }
            }
